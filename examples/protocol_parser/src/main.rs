...
fn parse_field(data: &[u8]) -> Result<(Field, &[u8]), Error> {
    ...
    let (fieldvalue, remainder) = match wire_type {
        WireType::Varint => {
            let (v, remainder) = parse_varint(remainder)?;
            (FieldValue::Varint(v), remainder)
        }
        WireType::I32 => (
            FieldValue::I32(i32::from_le_bytes(remainder[..4].try_into().unwrap())),
            &remainder[4..],
        ),
        WireType::Len => {
            let (len, remainder) = parse_varint(remainder)?;
            (
                FieldValue::Len(&remainder[..len as usize]),
